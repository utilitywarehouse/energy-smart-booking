...
type PointOfSaleSerialiser interface {
	Serialise(models.PointOfSaleCustomerDetails) ([]byte, error)
	Deserialise([]byte) (models.PointOfSaleCustomerDetails, error)
}
...
func (s *PointOfSaleCustomerDetails) GetByAccountNumber(ctx context.Context, accountNumber string) (*models.PointOfSaleCustomerDetails, error) {
	...
	unserialised, err := s.serialiser.Deserialise(details)
	if err != nil {
		return nil, fmt.Errorf("failed to deserialise details, %w", err)
	}

	return &unserialised, nil
}
...
func (s *PointOfSaleCustomerDetails) Upsert(ctx context.Context, accountNumber string, details models.PointOfSaleCustomerDetails) error {
	...
	serialisedDetails, err := s.serialiser.Serialise(details)
	if err != nil {
		return fmt.Errorf("failed to serialise model, %w", err)
	}
	...
	_, err = s.pool.Exec(ctx, `
		...
		DO UPDATE SET details = $2;`, accountNumber, serialisedDetails)
