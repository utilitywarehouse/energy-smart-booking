...
func (e *MeterpointEvaluator) GetElectricityMeterpointEligibility(ctx context.Context, mpan string, postcode string) (bool, string, error) {
	...
	for _, meter := range meters.Meters {
		if domain.IsElectricitySmartMeter(meter.MeterType.String()) {
			return false, "is already a smart meter", nil
		}
	}
	...
	if HasRelatedMPAN {
		return false, "related meterpoints present", nil
	}
	...
	if domain.HasComplexSSC(meters) {
		return false, "has complex SSC", nil
	}

	return true, "", nil
}

func (e *MeterpointEvaluator) GetGasMeterpointEligibility(ctx context.Context, mprn string) (bool, string, error) {
	...
	if domain.IsLargeCapacity(meters) {
		return false, "gas meterpoint is not large capacity", nil
	}
